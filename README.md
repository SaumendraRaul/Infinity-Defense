...
Open **Play.html** to play directly from this folder. No installation is needed. For the most reliable browser saves, use the hosted game or run `npm start` and open `http://127.0.0.1:4173`. The optional display fonts load online; system fonts are used offline.
...
The build produces a classic JavaScript bundle so the game also runs directly from local files. Edit source modules, then rebuild. The static distribution is `dist/`.
